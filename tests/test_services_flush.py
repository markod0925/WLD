...
def test_flush_coordinator_logs_stalled_queue_diagnostics(caplog) -> None:
    class _FakeStorage:
        def __init__(self) -> None:
            self.calls = 0
        def get_summary_job_status_counts(self): return {}
        ...
        def get_pending_counts(self):
            ...
            if self.calls >= 5:
                return {"text_segments": 0, "screenshots": 0, "intervals": 0, "key_events": 0, "processed_key_events": 0}
            return {"text_segments": 1, "screenshots": 1, "intervals": 1, "key_events": 0, "processed_key_events": 0}
    ...
    class _StalledSummarizer(_FakeSummarizer):
        def get_runtime_status(self):
            self._idx += 1
            pending_jobs = 0 if self._idx >= 5 else 1
            return {
                "queued_jobs": 1 if self._idx < 5 else 0,
                "running_jobs": 0,
                "pending_summary_jobs": pending_jobs,
                "has_unrecoverable_error": False,
                "summary_admission_paused": False,
    ...
    services = type("S", (), {"shutdown_event": type("E", (), {"is_set": lambda self: False})(), "summarizer": _StalledSummarizer(wait_values=[], running_values=[0]), "storage": _FakeStorage(), "state": _FakeState(), "keyboard_capture": _FakeKeyboard(), "text_service": _FakeTextService(), "error_notifier": _FakeNotifier()})()
    coordinator = FlushCoordinator(services, _FakeLifecycle(), 60, logging.getLogger(__name__))
    result = coordinator.flush_now("manual")
    assert result is not None
    assert any("event=summary_drain_stalled" in rec.message for rec in caplog.records)
