...
class SQLiteStorage:
    ...
    def purge_raw_data(self, start_ts: float, end_ts: float) -> list[str]:
        ...
        with self._lock:
            ...
            self._conn.execute(
                "DELETE FROM key_events WHERE ts >= ? AND ts <= ?",
                (start_ts, end_ts),
            )
            self._conn.execute(
    ...
    def get_pending_counts(self) -> dict[str, int]:
        with self._lock:
            intervals = int(self._conn.execute("SELECT COUNT(1) AS c FROM active_intervals WHERE summarized = 0 AND end_ts IS NOT NULL").fetchone()["c"])
            keys = int(self._conn.execute("SELECT COUNT(1) AS c FROM key_events").fetchone()["c"])
