...
class SQLiteStorage(ActivityRepository):
    ...
    def __init__(self, db_path: str) -> None:
        ...
        try:
            ...
            self.activity_repository = SQLiteActivityRepository(
                self._conn,
                self._lock,
                self.logger,
    ...
    def start_interval(self, info: ForegroundInfo, blocked: bool) -> int:
        interval_id = self.activity_repository.start_interval(info, blocked)
        self._record_db_write()
        return interval_id

    def close_interval(self, interval_id: int, end_ts: float) -> None:
        self.activity_repository.close_interval(interval_id, end_ts)
        self._record_db_write()
    ...
    def fetch_unsummarized_intervals(self, limit: int = 10000) -> list[ActiveInterval]:
        return self.activity_repository.fetch_unsummarized_intervals(limit)

    def fetch_unsummarized_blocked_intervals(self, limit: int = 10000) -> list[BlockedInterval]:
        return self.activity_repository.fetch_unsummarized_blocked_intervals(limit)

    def mark_intervals_summarized(self, start_ts: float, end_ts: float) -> None:
        self.activity_repository.mark_intervals_summarized(start_ts, end_ts)
        self._record_db_write()
